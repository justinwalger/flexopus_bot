...
async def test_booking_creation_requires_approval_then_runs_once_approved(chat_agent: ChatAgent):
    ...
    result = await run_turn(
        chat_agent,
        "Hallo, mein Name ist Martin Tester. Buch mir bitte den Schreibtisch "
        "mit der Bookable-ID 12 am Standort 3 von 2026-07-20T09:00 bis "
        "2026-07-20T10:00 Uhr, meine User-ID ist 42.",
        thread_id=thread_id,
    )
    ...
    assert action_request["args"]["bookable_id"] == 12
    assert action_request["args"]["location_id"] == 3
    assert action_request["args"]["user_id"] == 42
